...
def _seed_from_static(static_rows: list[RackAlloc]) -> None:
    ...
    STATE.last_event = (
        ...
        f"Dynamic Power {n_lps_cap()} racks [{rack_min_kw():.0f}–{STATE.max_rack_kw:.0f}] kW"
    )
    STATE.last_step = 0.0
...
def _step(samples: list[Sample], budget_kw: float) -> float:
    ...
    if added:
        STATE.last_event = (
            ...
            f"{len(STATE.pool_ids)} of {n_lps_cap()} Dynamic Power racks"
        )
    elif boosted:
        STATE.last_event = (
            f"Reallocated slack to {boosted} racks · "
            f"static {n_static_cap()} · Dynamic Power {n_lps_cap()}"
        )
