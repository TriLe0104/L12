...
def _inject_js(prefix: str, sess: dict[str, Any]) -> str:
    ...
    if view in ("kvm", "sol"):
        style = (
            ...
            "max-height:none!important;min-height:0!important;overflow:auto!important;background:#000;"
            "display:block!important}"
            "html.l12-console canvas{display:block;image-rendering:pixelated}"
            "</style>"
        )
    else:
    ...
    return (
        ...
        "document.querySelectorAll('header,.q-header,.q-drawer,.q-toolbar,nav,.navbar,.app-header').forEach(function(el){"
        "if(el.querySelector('canvas,#terminal-kvm,#terminal'))return;"
        ...
        "}"
        "function kvmCanvas(){"
        "var box=document.getElementById('terminal-kvm')||document.getElementById('noVNC_container')||document.getElementById('terminal');"
        "var list=(box||document).querySelectorAll('canvas');"
        ...
        "return best;"
        "}"
        "function scaleKvmCanvas(s){"
        "s=Number(s)||1;"
        "if(!(s>=1&&s<=3))s=1;"
        "var canvas=kvmCanvas();"
        "if(!canvas||canvas.width<8)return false;"
        "var box=document.getElementById('terminal-kvm')||canvas.parentElement||document.body;"
        "var bw=Math.max(1,box.clientWidth||window.innerWidth);"
        "var vh=(window.visualViewport&&window.visualViewport.height)||0;"
        "var bh=Math.max(1,vh||box.clientHeight||window.innerHeight);"
        "var fw=canvas.width,fh=canvas.height;"
        "var fit=Math.min(bw/fw,bh/fh);"
        "if(!(fit>0)||!isFinite(fit))fit=1;"
        "var z=fit*s;"
        "var w=Math.max(1,Math.round(fw*z)),h=Math.max(1,Math.round(fh*z));"
        ...
        "st.setProperty('image-rendering','pixelated','important');"
        "canvas.setAttribute('data-l12-zoom',String(z));"
        "var rfb=findRfb();"
        "if(rfb){"
        "hookClipboard(rfb);"
        "try{rfb.scaleViewport=false;}catch(e){}"
        ...
        "var fs=Math.max(12,Math.round(14*s));"
        "var term=findTerm();"
        "if(term){"
        ...
        "}"
        "document.querySelectorAll('.xterm,.xterm-rows,#terminal pre,#terminal .terminal').forEach(function(el){"
        ...
        "try{new MutationObserver(function(){l12applyScale(l12readScale());}).observe(document.documentElement,{childList:true,subtree:true});}catch(e){}"
        "(function tick(){scaleKvmCanvas(l12readScale());requestAnimationFrame(tick);})();"
        "setInterval(function(){l12applyScale(l12readScale());},500);"
        "})();</script>"
    )
