...
import { GB300_ELEVATION, POWER_LABEL, RUN_LABEL } from "@/lib/cluster";
  ...
  return (
    <>
      <tr className="inv-row" data-sel={expanded ? "true" : "false"} onClick={onToggle}>
        ...
        <td>{Math.round(rack.gpu_pct)}%</td>
        <td>{Math.round(rack.cpu_pct)}%</td>
        <td>{Math.round(rack.mem_pct)}%</td>
        <td>{rack.power_kw.toFixed(0)}</td>
      </tr>
      {expanded && (
