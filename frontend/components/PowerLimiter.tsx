...
function SummaryBars({
  ...
  const cols = [
    ...
    { key: "avail", label: "Available", kw: available, hint: "Allocated on racks but not consumed — Dynamic Power can move this" },
...
export function PowerLimiterPanel({
  ...
  const extraLabel =
    extraN > 0
      ? `${extraN} extra rack${extraN === 1 ? "" : "s"} vs static · ${data.dynamic.racks_enabled ?? 0} of ${nLps} Dynamic Power`
      : `Dynamic Power ${nLps} racks vs static ${nStatic}${gain > 0 ? ` · +${gain}` : ""}`;
    ...
    <section className="lps-row">
      <div className="lps-toolbar">
        <div>
          ...
          <span>
            {formatKw(data.total_budget_kw ?? data.budget_kw)} × {Math.round(data.threshold_pct ?? data.stay_under_pct ?? 80)}% · static {nStatic} · Dynamic Power {nLps}
      ...
      <article className="lps-panel" data-kind="dyn" data-active={data.mode === "dynamic"}>
        <Totals
          ...
          kicker={`${data.dynamic.racks_enabled ?? 0} of ${nLps} Dynamic Power racks · +${gain} vs static ${nStatic} · ${formatKw(envelope)} envelope`}
          used={dynUsed}
          available={dynAvail}
          floating={dynFloat}
          extra={extraLabel}
